...
{
    public class UpdateMaterialCommandHandler(ICoursesDbContext context) : IRequestHandler<UpdateMaterialCommand>
    {
        public async Task<Unit> Handle(UpdateMaterialCommand request, CancellationToken cancellationToken)
        {
            ...
            var entity = await context.Materials
                ...
                ?? throw new NotFoundException(nameof(Materials), request.Id);
